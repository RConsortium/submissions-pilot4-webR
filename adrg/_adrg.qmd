...
TODO: Revise to inlcude pkglite files instead of r4app.zip
...
In addition, create a new directory to hold the unpacked Pilot 4 Shiny application files. For demonstration purposes, the procedures below assume the new directory is this location: `C:\pilot4webR_files`.
...
Use the `pkglite` package to unpack the Shiny application bundle `pilot4_webR_pkglite.txt` as well as the environment lockfile contained in `pilot4_webR_env.txt` within the Pilot 4 eCTD submission transfer. These files are located in the following relative path within the eCTD transfer directory:

``` sh
m5\datasets\rconsortiumpilot4webR\analysis\adam\programs\pilot4_webR_pkglite.txt
m5\datasets\rconsortiumpilot4webR\analysis\adam\programs\pilot4_webR_env.txt
```

Enter the following command in the R console to extract the Shiny application files and environment lockfile to the appropriate destination directories:

``` r
# create application directory
dir.create("C:/pilot4webR_files/app")
pkglite::unpack(
  input = "C:/pilot4cwebR/m5/datasets/rconsortiumpilot4webR/analysis/adam/programs/pilot4_webR_pkglite.txt", 
  output = "C:/pilot4webR_files/app"
)

# extract environment lockfile
pkglite::unpack(
  input = "C:/pilot4cwebR/m5/datasets/rconsortiumpilot4webR/analysis/adam/programs/pilot4_webR_env.txt",
  output = "C:?pilot4webR_files" 
)
```
...
2.  Click the **Browse** button and navigate to the `C:/pilot3webR_files` directory to select the `submissions-pilot4-webr.Rproj` file. 
...
2.  Change the current directory to the `C:/pilot4webR_files` directory by running the following command (substitute the `C:/pilot3webR_files` location for your appropriate directory as needed):
...
Run the following code to compile the Shiny application source files to a new sub-directory `_site`  
